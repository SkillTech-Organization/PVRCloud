select * from
	(
	select * from 
	(select distinct NOD_FROM.NOD_ID as NOD_ID_FROM, NOD_TO.NOD_ID as NOD_ID_TO  from
		(select NOD_ID from PTP_PLANTOURPOINT PTP where TPL_ID = 12406) NOD_FROM
		inner join (select NOD_ID  
					from PTP_PLANTOURPOINT PTP 
					where TPL_ID = 12406) NOD_TO on NOD_TO.NOD_ID <> NOD_FROM.NOD_ID  
	) Q1, 
