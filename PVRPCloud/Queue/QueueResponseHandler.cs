...
namespace PVRPCloud.Queue;

public interface IQueueResponseHandler
{
    Task<ProjectRes<TourPoint>> Handle(string requestId);
}
...
public sealed partial class QueueResponseHandler : IQueueResponseHandler
{
    ...
    public QueueResponseHandler(IBlobHandler blobHandler, ILogger<QueueResponseHandler> logger, IRouteData routeData, IOptions<CommonSettings> commonSettings)
    {
    ...
    public async Task<ProjectRes<TourPoint>> Handle(string requestId)
    {
        ...
        bool IsFinishExist = _blobHandler.CheckIfBlobExist(_commonSettings.Value.CALC_CONTAINER_NAME, finishFile);
        bool IsResultExist = _blobHandler.CheckIfBlobExist(_commonSettings.Value.CALC_CONTAINER_NAME, resFile);
        bool IsExceptionExist = _blobHandler.CheckIfBlobExist(_commonSettings.Value.CALC_CONTAINER_NAME, excFile);
        bool IsProjectExist = _blobHandler.CheckIfBlobExist(_commonSettings.Value.CALC_CONTAINER_NAME, projFile);      //lehet,hogy hosszabb ideig tart a projekt mentése, ezért ellenőrizni kell a meglétét a felolvasás előtt
        ...
        if (IsFinishExist && IsResultExist && IsProjectExist)
        {
            ...
            List<Tour<TourPoint>> tours = [];
            List<Order> unplannedOrders = [];
            int currentTruck = 0;
            int currentRouteIndex = 0;
            Tour<TourPoint> currentTour = new();
            ...
            var projectRes = new ProjectRes<TourPoint>()
            {
    ...
    private async Task<PvrpData?> GetPvrpData(string requestId)
    {
        ...
        try
        {
            ...
            {
                var st = await _blobHandler.DownloadToStreamAsync(_commonSettings.Value.CALC_CONTAINER_NAME, brotliFileName);
                st.Flush();
                st.Position = 0;
    ...
    private async Task<string[]> GetResultFileFromBlob(string requestId)
    {
        ...
        using Stream stream = await _blobHandler.DownloadToStreamAsync(_commonSettings.Value.CALC_CONTAINER_NAME, fileName);
